...
def clean_en(
    ...
    merged_dir="en_zh_judgements.tsv",
    len_ratio_min=0.2,
    len_ratio_max=4.0,
    min_tgt_len=3,
    ...
    with open(illegal_log_name, "a", encoding="utf-8") as illegal_log:
        ...
        for lang in tqdm(languages):
            tsv_f = pd.read_csv(merged_dir, sep="\t").to_dict(orient="records")
            en_text = [x['en'] for x in tsv_f]
            lg_text = [x['trans'] for x in tsv_f]
            ...
            for en_sent, lg_sent, lbl in tqdm(zip(en_text, lg_text, labels)):
                en_s = en_sent.strip()
                lg_s = lg_sent.strip()
                print(ngram_jaccard(en_s, lg_s))
                if ngram_jaccard(en_s, lg_s) < ngram_overlap:
                    print('hhh')
                    print(en_sent, lg_sent)
